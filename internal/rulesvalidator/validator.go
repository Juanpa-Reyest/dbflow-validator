...
func (d *dockerRunner) RunValidator(ctx context.Context, req ValidatorContainerRequest) (string, error) {
	...
	// Copy report.zip from the container (generated by -zip-report flag).
	// This is a single file copy (reliable) that contains the full report:
	// HTML + CSS + JS + JSON. We extract it into ReportDirHostPath.
	if req.ReportZipContainerPath != "" && req.ReportZipHostPath != "" {
		rc, copyErr := c.CopyFileFromContainer(ctx, req.ReportZipContainerPath)
		if copyErr == nil {
			zipBytes, readErr := io.ReadAll(rc)
			rc.Close()
			if readErr == nil && len(zipBytes) > 0 {
				if mkdirErr := os.MkdirAll(filepath.Dir(req.ReportZipHostPath), 0o755); mkdirErr == nil {
					_ = os.WriteFile(req.ReportZipHostPath, zipBytes, 0o644)
					// Extract the ZIP into the report directory on the host.
					if req.ReportDirHostPath != "" {
						_ = extractZip(req.ReportZipHostPath, req.ReportDirHostPath)
					}
				}
			}
		}
	...
	// Also ensure the JSON report exists at ReportHostPath (backward-compat: ReadReportFile expects it).
	if req.ReportContainerPath != "" && req.ReportHostPath != "" {
		if _, statErr := os.Stat(req.ReportHostPath); statErr != nil {
...
// extractZip extracts a ZIP file into destDir, preserving directory structure.
