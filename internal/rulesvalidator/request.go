...
type ValidatorContainerRequest struct {
	...
	// ReportDirContainerPath is the container-side path to the full report/ directory
	// (contains JSON, HTML, and CSS/JS assets). Copied via Docker CopyFromContainer (tar).
	ReportDirContainerPath string
	// ReportDirHostPath is the host-side path where the full report directory is extracted.
	// Equals <cloneRoot>/src/main/resources/Validator/outputReport/report/
	ReportDirHostPath string
	// ReportZipContainerPath is the container-side path to the report.zip generated by -zip-report.
	ReportZipContainerPath string
	// ReportZipHostPath is the host-side path where the zip is written before extraction.
	ReportZipHostPath string
...
// containerReportDirSuffix is the suffix appended to ProjectRoot to get the full report directory
// (contains JSON, HTML, CSS/JS assets).
const containerReportDirSuffix = containerOutputReportSuffix + "/report"
...
func BuildContainerRequest(
	...
	return ValidatorContainerRequest{
		...
		// Full report directory (HTML + JSON + assets).
		ReportDirContainerPath: projectRoot + containerReportDirSuffix,
		ReportDirHostPath:      filepath.Join(cloneRoot, "src", "main", "resources", "Validator", "outputReport", "report"),
		// ZIP report (generated by -zip-report flag).
		ReportZipContainerPath: projectRoot + containerOutputReportSuffix + "/report.zip",
		ReportZipHostPath:      filepath.Join(cloneRoot, "src", "main", "resources", "Validator", "outputReport", "report.zip"),
		Env:                 env,
		User:                user,
	}
}
